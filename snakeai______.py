...
def choose_longest_safe_move(snake_head, pboard,snake):
    ...
    for i in mov:
        ...
        goto_where(goidx, i, 1)
        idxx = int((goidx[1]/point_size)*listsizeW + (goidx[0]/point_size))
        if possible_go(goidx) and pboard[idxx] > max and pboard[idxx]<UNDEFIND : 
            max = pboard[idxx]
            best_move = i
    return best_move
...
def virtual_shortest_move(snake_head):
    ...
    while not food_eated:
        ...
        #print(movedirection)
        goto_where(new_head , movedirection , 1)
        #print(movedirection)
...
def find_anyway(snake_head, pboard,snake):
    ...
    for i in mov:
        ...
        goto_where(goidx, i, 1)
        idxx = int((goidx[1]/point_size)*listsizeW + (goidx[0]/point_size))
        if possible_go(goidx) and pboard[idxx] < SNAKE and pboard[idxx] < min: 
            min = pboard[idxx]
            best_move = i
    return best_move
...
def main():
    ...
    #for pppppp in range(10):   
        ...
        #    print("%7d"%i,end=' ')
        #    j = j + 1
        #    if j == screenWidth / point_size :
        #        print("")
        #        j = 0
        print(snakeSegments)
        print(snake_head)
        j = 0;
        print(snakeSegments[-1])
        for i in mainboard:
            print("%7d"%i,end=' ')
            j = j + 1
            if j == screenWidth / point_size :
        ...
        #print(snakeSegments)
        # 移动
        #tmpsnake_head=snake_head[:]
        goto_where(snake_head,direction,1)
        # 判断是否吃掉了食物
        ...
        if snake_head[1] >= screenHeight or snake_head[1] < 0:
            gameOver(screen)
            flagg = 1
        if (snakeSegments[0] in snakeSegments[1:]):
            gameOver(screen)
            flagg = 1
        if flagg == 1 :
            return 
